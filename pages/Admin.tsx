...
export default function Admin() {
    ...
    const parseM3U = (content: string) => {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (line.startsWith('#EXTINF:')) {
                ...
                } else if (genero.includes('série') || genero.includes('series') || genero.includes('serie')) {
                    series.push({
                        title: currentItem.nome,
                        poster: currentItem.logo,
    ...
    const handleUpload = async () => {
        ...
        try {
            ...
            if (totalItems === 0) {
                throw new Error('Nenhum dado válido (filmes, séries ou canais) foi encontrado no arquivo. Verifique o formato.');
            }
            ...
            if (parsed.channels.length > 0) {
                ...
                if (channelsError) {
                    console.error('❌ Erro ao inserir canais:', channelsError);
                    throw channelsError;
                }
                console.log('✅ Canais inseridos:', channelsData?.length);
            ...
            if (parsed.movies.length > 0) {
                ...
                if (moviesError) {
                    console.error('❌ Erro ao inserir filmes:', moviesError);
                    throw moviesError;
                }
                console.log('✅ Filmes inseridos:', moviesData?.length);
            ...
            if (parsed.series.length > 0) {
                ...
                if (seriesError) {
                    console.error('❌ Erro ao inserir séries:', seriesError);
                    throw seriesError;
                }
                console.log('✅ Séries inseridas:', seriesData?.length);
            ...
            setProgress(90);
            console.log('📝 Registrando upload...');
            ...
            if (uploadError) {
                console.error('⚠️ Erro ao registrar upload:', uploadError);
            }
