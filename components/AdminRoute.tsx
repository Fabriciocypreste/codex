...
const ENABLE_DEV_PASSWORD_FALLBACK =
  import.meta.env.DEV && import.meta.env.VITE_ENABLE_ADMIN_PASSWORD_FALLBACK === 'true';
...
const AdminRoute: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const canUsePasswordFallback = ENABLE_DEV_PASSWORD_FALLBACK && !!ADMIN_PASSWORD;

  if (canUsePasswordFallback && authenticated) {
    return <>{children}</>;
  }
        ...
        <div className="w-full max-w-md">
          ...
          <div className="bg-[#121217] border border-white/10 rounded-2xl p-8 shadow-2xl">
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                <h2 className="text-lg font-bold text-white">Fallback de Desenvolvimento</h2>
                <p className="text-white/40 text-xs">Use somente em ambiente local controlado</p>
              </div>
            </div>
            ...
            <p className="text-white/20 text-xs text-center mt-4">Logado como: {user.email}</p>
          </div>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen bg-[#0B0B0F] flex items-center justify-center p-4">
      <div className="w-full max-w-md bg-[#121217] border border-red-600/30 rounded-2xl p-8 shadow-2xl text-center">
        ...
        <p className="text-white/60 text-sm">
          Sua conta não possui role <code>admin</code> em <code>app_metadata</code>.
        </p>
      </div>
    </div>
  );
